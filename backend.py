...
class Backend(BackendBase):
    def __init__(self):
        ...
        self._is_reconnecting: bool = (
            False  # Prevent multiple simultaneous reconnect attempts
        )
    ...
    def setup_client(self):
        ...
        try:
            ...
            log.error(f"Failed to setup discord client: {ex}")
    ...
    def set_mute(self, muted: bool):
        if not self._ensure_connected():
            log.warning("Cannot set mute: Discord client not connected")
            return
        self.discord_client.set_voice_settings({"mute": muted})

    def set_deafen(self, muted: bool):
        if not self._ensure_connected():
            log.warning("Cannot set deafen: Discord client not connected")
            return
        self.discord_client.set_voice_settings({"deaf": muted})

    def change_voice_channel(self, channel_id: str = None) -> bool:
        if not self._ensure_connected():
            log.warning("Cannot change voice channel: Discord client not connected")
            return False
    ...
    def change_text_channel(self, channel_id: str) -> bool:
        if not self._ensure_connected():
            log.warning("Cannot change text channel: Discord client not connected")
            return False
    ...
    def set_push_to_talk(self, ptt: str) -> bool:
        if not self._ensure_connected():
            log.warning("Cannot set push to talk: Discord client not connected")
            return False
    ...
    def _get_current_voice_channel(self):
        if not self._ensure_connected():
            log.warning(
                "Cannot get current voice channel: Discord client not connected"
            )
            return
    ...
    def _ensure_connected(self) -> bool:
        ...
        Does not trigger reconnection to avoid recursive calls.
        """
        if self.discord_client is None:
            log.warning("Discord client not initialized")
            return False
        if not self.discord_client.is_connected():
            log.warning("Discord client not connected")
            return False
        if not self._is_authed:
            log.warning("Discord client not authenticated")
            return False
        return True
