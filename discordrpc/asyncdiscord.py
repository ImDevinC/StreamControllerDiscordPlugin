...
from .constants import MAX_SOCKET_CONNECT_RETRIES, OAUTH_TOKEN_TIMEOUT_SEC
...
class AsyncDiscord:
    ...
    def connect(self, callback: callable):
        tries = 0
        while tries < MAX_SOCKET_CONNECT_RETRIES:
            log.debug(
                f"Attempting to connect to socket, attempt {tries + 1}/{MAX_SOCKET_CONNECT_RETRIES}"
            )
            self.rpc.connect()
        ...
        try:
            ...
            log.error(f"Invalid JSON response from Discord: {ex}")
            raise RPCException
